import { StytchClientOptions, SessionTokens, UnsubscribeFunction, ConsumerState, B2BState, User, Member, SessionTokensUpdate, Organization, StytchProjectConfigurationInput } from '@stytch/core/public';
import { IConsumerSubscriptionService, IB2BSubscriptionService, ISubscriptionService, IfOpaqueTokens, ExtractOpaqueTokens, AllowedOpaqueTokens } from '@stytch/core';
...
export declare class SubscriptionDataLayer<T extends ConsumerState | B2BState> {
    ...
    writeSessionCookie(stateDiff: StateWithReadableTokensLoggedIn<T>): void;
...
type StateWithIntermediateSessionToken = StateWithReadableIntermediateSessionToken | StateWithOpaqueIntermediateSessionToken;
type StateWithTokensDiff<T> = StateWithTokensLoggedIn<T> | StateWithTokensLoggedOut | StateWithIntermediateSessionToken;
export declare class BaseSubscriptionService<T extends ConsumerState | B2BState, TOpaqueTokens extends AllowedOpaqueTokens> implements ISubscriptionService<T, TOpaqueTokens> {
     ...
     */
    private fromCache;
    constructor(_publicToken: string, _datalayer: SubscriptionDataLayer<T>, { usingCustomApiEndpoint }: {
        usingCustomApiEndpoint: boolean;
    });
    private _listen;
...
export declare class ConsumerSubscriptionService<TProjectConfiguration extends StytchProjectConfigurationInput> extends BaseSubscriptionService<ConsumerState, ExtractOpaqueTokens<TProjectConfiguration>> implements IConsumerSubscriptionService<TProjectConfiguration> {
    updateUser: (user: User) => void;
    getUser: () => User | null;
    getSession: () => import("@stytch/core/public").Session | null;
    updateSession: IConsumerSubscriptionService<TProjectConfiguration>['updateSession'];
}
export declare class B2BSubscriptionService<TProjectConfiguration extends StytchProjectConfigurationInput> extends BaseSubscriptionService<B2BState, ExtractOpaqueTokens<TProjectConfiguration>> implements IB2BSubscriptionService<TProjectConfiguration> {
    updateMember: (member: Member) => void;
    getMember: () => Member | null;
    updateOrganization: (organization: Organization) => void;
    getOrganization: () => Organization | null;
    getSession: () => import("@stytch/core/public").MemberSession | null;
    updateSession: IB2BSubscriptionService<TProjectConfiguration>['updateSession'];
}
export {};

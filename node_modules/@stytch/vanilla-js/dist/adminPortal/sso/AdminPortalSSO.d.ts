import { StytchProjectConfigurationInput } from '@stytch/core/public';
import React from 'react';
import { MappedPropsFromRouteMap } from '../Router';
import { AdminPortalComponentMountOptions } from '../makeAdminPortalComponentMountFn';
import { AdminPortalUIConfigRoleDisplayProvider } from '../utils/AdminPortalUIConfigRoleDisplayProvider';
/**
 * The UI configuration object for SSO configuration used in the Admin Portal.
 */
export type AdminPortalSSOUIConfig = AdminPortalUIConfigRoleDisplayProvider;
export interface AdminPortalSSOMountOptions<TProjectConfiguration extends StytchProjectConfigurationInput = Stytch.DefaultProjectConfiguration> extends AdminPortalComponentMountOptions<TProjectConfiguration> {
    /**
     * An {@link AdminPortalSSOUIConfig} object containing custom configuration.
     */
    config?: AdminPortalSSOUIConfig;
}
declare const routeMap: {
    connectionDetails: ({ connectionId }: {
        connectionId: string;
    }) => React.JSX.Element;
    connectionsList: () => React.JSX.Element;
    newConnection: () => React.JSX.Element;
    newConnectionConfigure: ({ connection: connectionProp, }: {
        connection: import("./TaggedConnection").OIDCTaggedConnection | import("./TaggedConnection").SAMLTaggedConnection;
    }) => React.JSX.Element;
};
export type SsoRouterMappedProps = MappedPropsFromRouteMap<typeof routeMap>;
export declare const Content: () => React.JSX.Element;
export declare const AdminPortalSSO: <TProjectConfiguration extends Partial<import("@stytch/core/public").StytchProjectConfiguration>>(props: AdminPortalSSOMountOptions<TProjectConfiguration>) => React.JSX.Element;
export {};
